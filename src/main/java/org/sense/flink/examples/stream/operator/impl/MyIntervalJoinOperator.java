...
import org.sense.flink.examples.stream.operator.AbstractMyIntervalJoinOperator;
 ...
 *
 * <p>
 * By using a configurable lower and upper bound this operator will emit exactly
 * those pairs (T1, T2) where t2.ts ∈ [T1.ts + lowerBound, T1.ts + upperBound].
 * Both the lower and the upper bound can be configured to be either inclusive
 * or exclusive.
 *
 * <p>
 * As soon as elements are joined they are passed to a user-defined
 * {@link ProcessJoinFunction}.
 *
 * <p>
 * The basic idea of this implementation is as follows: Whenever we receive an
 * element at {@link #processElement1(StreamRecord)} (a.k.a. the left side), we
 * add it to the left buffer. We then check the right buffer to see whether
 * there are any elements that can be joined. If there are, they are joined and
 * passed to the aforementioned function. The same happens the other way around
 * when receiving an element on the right side.
 *
 * <p>
 * Whenever a pair of elements is emitted it will be assigned the max timestamp
 * of either of the elements.
 *
 * <p>
 * In order to avoid the element buffers to grow indefinitely a cleanup timer is
 * registered per element. This timer indicates when an element is not
 * considered for joining anymore and can be removed from the state.
 *
 * @param <K>
 *            The type of the key based on which we join elements.
 * @param <T1>
 *            The type of the elements in the left stream.
 * @param <T2>
 *            The type of the elements in the right stream.
 * @param <OUT>
 *            The output type created by the user-defined function.
 */
@Internal
public class MyIntervalJoinOperator<K, T1, T2, OUT> extends AbstractMyIntervalJoinOperator<K, T1, T2, OUT> {

	private static final long serialVersionUID = -5380774605111543454L;

	private static final Logger logger = LoggerFactory.getLogger(MyIntervalJoinOperator.class);
	 ...
	 *
	 * @param lowerBound
	 *            The lower bound for evaluating if elements should be joined
	 * @param upperBound
	 *            The upper bound for evaluating if elements should be joined
	 ...
	 *            upper bound
	 * @param udf
	 *            A user-defined {@link ProcessJoinFunction} that gets called
	 *            whenever two elements of T1 and T2 are joined
	 */
	public MyIntervalJoinOperator(long lowerBound, long upperBound, boolean lowerBoundInclusive,
			boolean upperBoundInclusive, TypeSerializer<T1> leftTypeSerializer, TypeSerializer<T2> rightTypeSerializer,
	...
	public void open() throws Exception {
		...
		internalTimerService = getInternalTimerService(CLEANUP_TIMER_NAME, StringSerializer.INSTANCE, this);
	}
	...
	public void initializeState(StateInitializationContext context) throws Exception {
		...
		this.leftBuffer = context.getKeyedStateStore().getMapState(new MapStateDescriptor<>(LEFT_BUFFER,
				LongSerializer.INSTANCE, new ListSerializer<>(new BufferEntrySerializer<>(leftTypeSerializer))));

		this.rightBuffer = context.getKeyedStateStore().getMapState(new MapStateDescriptor<>(RIGHT_BUFFER,
				LongSerializer.INSTANCE, new ListSerializer<>(new BufferEntrySerializer<>(rightTypeSerializer))));
	}

	/**
	 * Process a {@link StreamRecord} from the left stream. Whenever an
	 * {@link StreamRecord} arrives at the left stream, it will get added to the
	 * left buffer. Possible join candidates for that element will be looked up from
	 * the right buffer and if the pair lies within the user defined boundaries, it
	 * gets passed to the {@link ProcessJoinFunction}.
	 *
	 * @param record
	 ...
	 * {@link StreamRecord} arrives at the right stream, it will get added to the
	 * right buffer. Possible join candidates for that element will be looked up
	 * from the left buffer and if the pair lies within the user defined boundaries,
	 * it gets passed to the {@link ProcessJoinFunction}.
	 *
	 * @param record
	 *            An incoming record to be joined
	 * @throws Exception
	...
	private <THIS, OTHER> void processElement(final StreamRecord<THIS> record,
			final MapState<Long, List<MyIntervalJoinOperator.BufferEntry<THIS>>> ourBuffer,
			final MapState<Long, List<MyIntervalJoinOperator.BufferEntry<OTHER>>> otherBuffer,
			final long relativeLowerBound, final long relativeUpperBound, final boolean isLeft) throws Exception {
	...
	private static <T> void addToBuffer(final MapState<Long, List<MyIntervalJoinOperator.BufferEntry<T>>> buffer,
			final T value, final long timestamp) throws Exception {
	 ...
	 *
	 * <p>
	 * It gives access to the timestamps of the left element in the joined pair, the
	 * right one, and that of the joined pair. In addition, this context allows to
	...
	/**
	 * A container for elements put in the left/write buffer. This will contain the
	 * element itself along with a flag indicating if it has been joined or not.
	 */
	@Internal
	@VisibleForTesting
	...
	/**
	 * The {@link CompositeTypeSerializerConfigSnapshot configuration} of our
	 * serializer.
	 *
	 * @deprecated this snapshot class is no longer in use, and is maintained only
	 *             for backwards compatibility. It is fully replaced by
	 *             {@link BufferEntrySerializerSnapshot}.
	 */
	@Deprecated
	public static class BufferSerializerConfigSnapshot<T>
		...
		public TypeSerializerSchemaCompatibility<BufferEntry<T>> resolveSchemaCompatibility(
			...
			return CompositeTypeSerializerUtil.delegateCompatibilityCheckToNewSnapshot(newSerializer,
					new BufferEntrySerializerSnapshot<>(), getSingleNestedSerializerAndConfig().f1);
		}
	}
