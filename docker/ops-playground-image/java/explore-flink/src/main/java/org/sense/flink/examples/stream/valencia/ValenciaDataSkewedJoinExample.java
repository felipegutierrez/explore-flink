...
public class ValenciaDataSkewedJoinExample {
    ...
    public ValenciaDataSkewedJoinExample(String ipAddressSource, String ipAddressSink) throws Exception {
        this(ipAddressSource, ipAddressSink, true);
    }

    public ValenciaDataSkewedJoinExample(String ipAddressSource, String ipAddressSink, boolean offlineData) throws Exception {
        ...
        DataStream<ValenciaItem> streamAirPollution = env
                .addSource(new ValenciaItemConsumer(ValenciaItemType.AIR_POLLUTION, frequencyMilliSeconds1, collectWithTimestamp, offlineData, skewedDataInjection, Long.MAX_VALUE, false)).name(METRIC_VALENCIA_SOURCE + "-" + ValenciaItemType.AIR_POLLUTION)
                .map(new ValenciaItemDistrictMap()).name(METRIC_VALENCIA_DISTRICT_MAP)
